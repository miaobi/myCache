...
public class TreeHelper {
    ...
    public TreeHelper(TreeNode root, List<TreeNode> treeNodeList) {
        this.root = root;
     ...
     */
    public static List<TreeNode> changeEnititiesToTreeNodes(List<Category> entityList) {
        Category orgEntity = new Category();
        List<TreeNode> tempNodeList = new ArrayList<TreeNode>();
        TreeNode treeNode;

        Iterator it = entityList.iterator();
        while (it.hasNext()) {
            orgEntity = (Category) it.next();
            treeNode = new TreeNode();
            treeNode.setObj(orgEntity);
            treeNode.setParentId(orgEntity.getParentId());
            treeNode.setSelfId(orgEntity.getCatId());
            treeNode.setNodeName(orgEntity.getCatName());
